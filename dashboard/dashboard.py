...
def create_cluster(df):
    ...
    return weekday_usage
def create_cluster2(df):
    bins = [0, 1000, 3000, 5000, max(df["cnt"])]
    labels = ["Rendah", "Sedang", "Tinggi", "Sangat Tinggi"]
    
    df["kategori_pemakaian"] = pd.cut(df["cnt"], bins=bins, labels=labels)
...
cluster2_df = create_cluster2(main_df)
...
# Visualisasi Distribusi Kategori Pemakaian
ss = plt.figure(figsize=(6, 4))
sns.countplot(x="kategori_pemakaian", data=all_df, palette="coolwarm")
plt.title("Distribusi Kategori Pemakaian Sepeda")
plt.xlabel("Kategori Pemakaian")
